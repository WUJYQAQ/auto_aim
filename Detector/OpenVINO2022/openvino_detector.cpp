...
void OpenVINODetector::display(cv::Mat &image2show, ArmorObject object) {
    // 绘制十字瞄准线
    line(image2show, cv::Point2f(image2show.size().width / 2, 0), cv::Point2f(image2show.size().width / 2, image2show.size().height), {0,255,0}, 1);
    line(image2show, cv::Point2f(0, image2show.size().height / 2), cv::Point2f(image2show.size().width, image2show.size().height / 2), {0,255,0}, 1);
    ...
    circle(image2show, cv::Point((object.pts[0].x+object.pts[2].x)/2, (object.pts[0].y+object.pts[2].y)/2), 5, cv::Scalar(255, 255, 255), 5);
    ...
    switch (object.color) {
        case ArmorColor::BLUE:
            cv::putText(image2show, "Blue_" + std::to_string(static_cast<int>(object.number)), cv::Point(box_top_x + 2, box_top_y), cv::FONT_HERSHEY_TRIPLEX, 1,
                        cv::Scalar(255, 0, 0));
            break;
        case ArmorColor::RED:
            cv::putText(image2show, "Red_" + std::to_string(static_cast<int>(object.number)), cv::Point(box_top_x + 2, box_top_y), cv::FONT_HERSHEY_TRIPLEX, 1,
                        cv::Scalar(0, 0, 255));
            break;
        case ArmorColor::NONE:
            cv::putText(image2show, "None_" + std::to_string(static_cast<int>(object.number)), cv::Point(box_top_x + 2, box_top_y), cv::FONT_HERSHEY_TRIPLEX, 1,
                        cv::Scalar(0, 255, 0));
            break;
        case ArmorColor::PURPLE:
            cv::putText(image2show, "Purple_" + std::to_string(static_cast<int>(object.number)), cv::Point(box_top_x + 2, box_top_y), cv::FONT_HERSHEY_TRIPLEX, 1,
                        cv::Scalar(255, 0, 255));
            break;
        default:
            break;
